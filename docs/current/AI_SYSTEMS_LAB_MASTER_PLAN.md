...
alternative if M0 shows degenerate truncations).
...
|---|---|---|---|---|---|---|---|
| **M0** | Truncation diagnostic + telemetry floor + $4 GPU benchmark | Are cap-hits convergent-but-truncated, answer-formed-but-cut, or degenerate? What caps fit the 5080? What is real rented 3090/5090 decode? | Decides R7's fate for ~half a day + ~$4; autopsy's top telemetry fixes land before any long run | none | ~½ day, ≤36 TRAIN re-runs, no DEV/TEST | Truncations mostly convergent → R7 GO; degenerate → R7 re-scoped/dropped, R9 up | R7 decision, hardware benchmark number, GPU-hours ledger |
