...
1. **Every required ID must be generated by that scenario** — otherwise 100% recall
   ...
   (`test_required_evidence_actually_exists_in_the_world`)
2. **Every required ID must be reachable by the tool set.** The evidence plan is
   ...
   `provider_event_id`, which is unknowable until a settlement has been read.

> Adding a scenario class means checking both. Violating (2) produces a plausible
> but permanently unwinnable case — the failure mode that cost the most time so far.
