...
SHA, `/proc` server args, and the running executable + mapped shared objects match the
registered execution system; a local llama.cpp arm cannot be run without `--candidate`.
...
| (this commit) | § 9–11 (appended, nothing above § 17 edited) | **Pre-DEV contract/gate review (Fable, review B) — no blockers; follow-ups landed before the first DEV call.** (1) *Erratum:* § 11 lists Qwen3-8B DEV "no-output 1"; under § 11's own definition (no parsable answer) Qwen3-8B DEV no-output is **9** (8 schema/parse + 1 cap hit) — "1" is its cap-hit count. The modern-small clause "no-output ≤ 8/48" therefore asks for slightly *better* reliability than the control; the thresholds were fixed at `b2f85d5` before any pilot and stand as written; `GATES["historical"]` is left untouched so the gates digest is unchanged (docstring corrected). (2) *Transcription:* § 10 Qwen3.8 Q3_K_M cap table reads `{4096: 33 …}`; the registry payload and `cap-calibration` say **32** (outcome unchanged). (3) *Enforcement:* a resumed DEV/TEST run counts `cases_done` over all its ledger end lines; `DEV_QUALIFIED`/`DEV_REJECTED` require `gate_evaluation.qualified` to match the edge and `gate_evaluation.gates_digest` to equal the frozen digest, and the verdict is re-derived from `dev_result.json` (`metrics` + `reference`) with `evaluate_gates`; DEV/TEST refuse a server session that any earlier ledger line has seen (a "fresh session" = a `local_server_session` in no earlier ledger line; the prime is `make r6-prime`: content `prime`, 8 tokens, temp 0, seed 42). (4) *Precise wording:* "p50 wall" is compared as `int(statistics.median(wall_ms))` (thresholds 38,592 and 40,380 = floor of 3 × 12,864 and 0.75 × 53,840.5); "resident GPU memory" = max over the run-start sample and the pre-case `gpu_mem_used_mib_now` samples on the DEV trajectories, card total from `nvidia-smi memory.used` with the idle baseline (1,734 MiB) reported alongside, no other GPU consumer started during DEV; "A dominates B" iff B-only = 0 and A-only ≥ 1 on the same split, "operationally dominates" additionally requires p50 ≤ and resident memory ≤, "complementary" iff both-only ≥ 1; the § 9 probe criterion counts a case identical when its `output_digest` matches or both runs are no-output with identical `stop_reason` and `output_tokens`. (5) *Materiality wording:* the same-config restart pair `R3-qwen-dev`/`R3b-qwen4096-dev` moved 6/5 all-pass cases (net +1); +5 net is above that noise, not far above it. (6) *Disclosure:* Nemotron's historical DEV latency/memory were measured under `--fit on --fit-target 1024 --no-mmap`, partially CPU-offloaded, sharing the card with the resident Qwen3-8B server; R6 candidates run alone, fully offloaded — this favours candidates on the alt-branch latency clause (moot for Qwen3.8: TRAIN p50 288 s ≫ 40 s) and on the Bonsai fallback; Bonsai additionally runs on an older llama.cpp base (`prism-b9596`, ggml 0.13.1), whose sampler/grammar/reasoning-parsing code is part of its execution system. (7) *Fallback pre-registration:* if the Bonsai efficiency gate has no Qwen3.8 DEV numbers, the Nemotron reference resident memory is measured once before Bonsai DEV by serving Nemotron alone with the historical `serve-nemotron.sh` flags (`FIS_NEMOTRON_FIT_TARGET_MIB=1024`), priming, sampling `nvidia-smi memory.used`, stopping; if that too is unavailable, Qwen3.8's TRAIN-recorded 15,682 MiB is used. (8) *Provenance note:* every (repo, revision, file) triple's HF LFS oid was read via the HF API on 2026-08-18 and equals the local SHA-256 (LFS oid = sha256): Qwen3-8B `d98cdcbd…`, Nemotron `c7be5d2c…` @ `be042bfc`, Qwen3.5-9B `03b74727…` @ `99a1b218`, Qwen3.8 Q3_K_M `7f3b845b…` @ `fa83dc31`, UD `00cf92e6…` @ `1cff334a`, Bonsai `868c1171…` @ `abbae723` (an operator-recorded check; the registry stores the local SHA); the Qwen3-8B record's `source_url` points at `blob/main`, its `source_revision` is the pin. Qwen3.8 cross-session probe numbers: 8 identical output digests + 4 both-no-output (identical `stop_reason`/`output_tokens`), all_pass 12/12. (9) *Verdict reporting:* a candidate that fails a gate on one clause while passing others is reported per clause (e.g. "DEV_REJECTED on p50 latency; all-pass clause met"), never collapsed to a scalar. |

---

> **STATUS (appended 2026-08-20): FROZEN CONTRACT — binding record of a completed
> experiment.** This file is bound by git blob SHA in the provenance registry
> (`committed_contract_extends`: append-only after freeze — never edit above this
> line). It governs the frozen R6 baseline; it is not a plan. New experiments use
