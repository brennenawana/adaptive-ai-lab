...
**Answers in one line each.** *Qwen3-8B* is now a historical control only (§ 2). *Qwen3.5-9B* meets the DEV half of the pre-registered small-tier materiality definition (+6 all-pass ≥ +5, equal to Nemotron's DEV) but fails the reliability and latency clauses (10 cap hits, 4.5× Qwen3-8B's wall), so the TEST half was never tested — a real DEV quality gain, not a drop-in small-tier replacement at this reasoning budget (§ 5.1). *Q3_K_M* won the TRAIN-only quant selection on quality (§ 4). *Qwen3.8-27B* is the strongest local on DEV (25/48) and within one case of Nemotron on TEST (47 vs 48/96); it is **not** better than Nemotron on all-pass but is categorically better on silent-failure burden (2 vs 27 on TEST) and therefore on the unchanged R4 cascade (93/96 vs 69/96); it is slower (265 s vs 56 s p50) and uses the whole card; the two are complementary, not dominated (§ 5.2, § 7). *Nemotron* retains unique TEST successes (12 against Qwen3-8B + Qwen3.8, 6 once Bonsai is in the set), concentrated in S10/S06/S11 where Qwen3.8 never finishes (§ 7). *Bonsai* is an efficiency point by the contract's DEV definition (0.61× memory, 0.30× p50 of Qwen3.8, ≥ floor); on TEST 34/96 at 0.29× p50 with resident memory drifting to 10.2 GiB max = 0.67× (above the DEV gate ratio), and it carries the largest silent-failure burden on DEV (21) and the second-largest on TEST (41, behind Qwen3-8B's 47) — the historical evidence-citation failure shape (§ 5.3, § 9). The cheapest sufficient oracle structure changes: with Qwen3.8 under the *unchanged* R4 verifier gate nearly every residual failure is visible, so the 93/96 system needs no learned router (§ 8). Remaining silent failures are evidence-citation failures in the 9B/Bonsai tier and two in Qwen3.8 (§ 9). The evidence favours **a reasoning-budget (cap/context) milestone on the frozen Qwen3.8-27B execution system** as the single next step — not QLoRA on Qwen3.8 on this evidence and not a new router; Nemotron specialization remains the live alternative if R7 fails to convert truncations (§ 11). New inference: 516 local cases, **0 frontier calls**; wall ≈ 24.3 h from `/goal` to the final commit (§ 10).
...
all-pass **25/48 ≥ 23** (the +2 over Nemotron is inside the contract's own +5 noise band; TEST makes it 47 vs 48). Failure shape unlike any earlier arm: 22 no-outputs, all cap hits at 8 192 output tokens (10 of them already inside the JSON answer after reasoning had ended; S07/S10/S11/S12 almost never finish), and **one** verifier-clean wrong answer (`S08-2000007`); 25 of the 26 completed answers passed. p50 263 s (30 tok/s, dense 27B at Q3), 15.1 GiB resident. The alternative branch (p50 ≤ 40.4 s or no-output ≤ 6) was not met — it qualifies on quality alone.
...
**Pre-registered readings (contract § 11 materiality).** Qwen3.8-27B vs Nemotron: 47 vs 48/96 → **competitive** (within ±4), not "better"; pairwise 29 both / 19 Nemotron-only / 18 Qwen3.8-only / 30 neither → complementary. Qwen3.8 vs Qwen3-8B: 30 Qwen3.8-only vs 10 Qwen3-8B-only (paired difference +20 — far above the +8 bar that was pre-registered for the small-tier candidate, applied here descriptively). Bonsai on TEST: 34/96 — below Nemotron by 14 and Qwen3.8 by 13, above Qwen3-8B by 7 (pairwise vs Qwen3-8B 14 both / 13 / 20 / 49); no pre-registered materiality reading applied to the efficiency role beyond its DEV gate; it is complementary with every local arm (vs Qwen3.8: 20 both / 27 / 14 / 35; vs Nemotron 28 / 20 / 6 / 42). The frontier dominates every local arm (each local's pass set ⊂ frontier's, one `S12-3002011` case fails everywhere).

## 7. Unchanged R4, post-answer oracle, overlap, unique rescue, tier coverage, dominance

**R4 (verifier cascade, unchanged, replayed against the frozen frontier rows).** The gate escalates on no-output / verifier failure — exactly Qwen3.8's failure mode — so on TEST Qwen3.8 + R4 reaches **93/96** (FN 2, utilization 49.0 %, 0 unnecessary, $0.0628/success) against Nemotron + R4 69/96 (FN 26, util 21.9 %, $0.0380) and Qwen3-8B + R4 48/96 (FN 47). The price is frontier utilization (47 calls vs 21) and wall time (p50 268 s incl. escalations). $/success is the reference frontier spend only (local compute priced 0; frontier input tokens under-reported by the CLI → a floor), so the Qwen3.8 cascade costs at least $0.025/success more than Nemotron's and the figure is blind to its 4.7× local wall. The **post-answer oracle** (escalate exactly the local failures) is 95/96 for every arm; Qwen3.8's oracle gap is 2 cases at the same utilization (47 vs 48 calls) — R4 is already the oracle for it, because its failures are visible. Bonsai + R4 reaches only 55/96 (FN 40, util 21.9 %): 41 of its 62 failures are verifier-clean wrong answers the gate cannot see — the efficiency tier inherits the historical silent-failure shape (29 evidence-only, 12 root cause) and adds 13 unsupported-claim verifier failures.

**Unique successes on TEST:** against the two historical locals (the set the R5 report used) Qwen3.8 has 15 (S01 ×7, S04 ×4, S03, S06 ×2, S09 — the classes every earlier local scored 0–1 on) and Nemotron 12 (S10 ×5, S06 ×3, S11 ×2, S02, S03); in the full four-local set Qwen3.8 13, Nemotron 6, Qwen3-8B 3, Bonsai 3 — Nemotron's unique set halves once Bonsai is counted (Bonsai covers several S10/S11 cases). **Dominance:** no local arm's pass set contains another's on either split; every pair is complementary; only the frontier dominates. Qwen3.8 does not *operationally* dominate Nemotron either (slower; resident memory not comparable — Nemotron's footprint was never re-measured in R6) — it wins on silent-failure burden and loses on latency.
...
With Qwen3.8-27B as the local tier, the oracle is the deterministic verifier gate itself: local answers that finish are almost always right (TEST: 47 pass of 49 finished); answers that do not finish are visibly no-output and escalate. 93/96 at 49 % frontier utilization, 0 unnecessary escalations, 2 false negatives (`S03-3000002`, `S11-3001010`, both evidence-only). A two-local cascade with the deterministic R4 gate at both stages (Qwen3.8 → Nemotron → frontier) replays on TEST to **77/96 with FN 18 and 12 frontier calls (12.5 %)** against 93/96, FN 2, 47 calls for Qwen3.8 → frontier: −16 all-pass for −35 frontier calls, because Nemotron's 27 silent failures pass the gate (the reverse order, Nemotron → Qwen3.8 → frontier, gives 68/96, worse than Nemotron + R4 alone); on DEV 41/48 (FN 7, 16.7 %) vs 47/48. The lowest-burden *sufficient* structure is therefore the single-local Qwen3.8 + R4 cascade; its burden is latency (265 s local p50) and VRAM (15.1 GiB), not routing complexity.
...
Run wall (ledger end lines, h): Qwen3.5 pilot 0.72, probe 0.24, DEV 0.81; Qwen3.8 Q3_K_M pilot 2.58, probe 0.71, DEV 3.03, TEST 6.20; UD pilot 1.81; Bonsai pilot 0.81, probe 0.10 (+ the unrecorded 8-case first part), DEV 1.13, TEST 2.33 — **20.5 h of model time**. Wall by phase (`phases.jsonl`): R6.0 bootstrap ~1.1 h (before `/goal`); R6.1 provenance 21:00–21:36 UTC (0.6 h); R6.2 TRAIN 8.2 h; R6.3 freeze + reviews 0.3 h; R6.4 DEV 5.5 h; R6.5 TEST 9.3 h; R6.6–7 analysis, report, docs, review C and its fixes 20:48–21:20 UTC (0.5 h) — **≈ 24.3 h** from `/goal` (20:59:46 UTC 08-18) to the final commit. Model time dominates; the Qwen3.8 arms (27B at 30 tok/s, ~8k tokens per case) account for 14.3 h of the 20.5. Process note: § 6–11 and the doc updates were drafted during the Bonsai TEST run and filled from `analysis/test.json` after it ended (20:48–20:51 UTC); review C followed and its changes were applied before the final commit.

## 11. Residual failure mechanisms of the strongest modern local, and the one next milestone

Qwen3.8-27B Q3_K_M's residual failures are almost entirely **reasoning-budget truncations** — 47 of 49 TEST failures and 22 of 23 DEV failures are `stop_reason=length` at 8 192 output tokens (23–36k reasoning characters), concentrated in S07, S10, S11, S12 (0/8 each on TEST) and S06/S02. In 16/47 TEST and 10/22 DEV of them the model had already left reasoning and was cut inside the JSON answer (0.2–3.4k content chars); in the rest it was still reasoning. Whether the long reasoning is genuine or degenerate cannot be told from stored fields (the reasoning text is not persisted; chars-per-token shows no gross anomaly). The median *completed* answer is 5.9k output tokens on TEST (5.4k on DEV; the run p50 of 7.9k is pulled up by the capped cases). When it finishes it is right (TEST 47/49, DEV 25/26) — but the finished subset is also the easier one (Nemotron passes 61 % of Qwen3.8's finished TEST cases vs 38 % of its unfinished ones), so 96 % is an upper bound on what a larger budget would convert; that conversion rate is R7's measurand, not a given. This is not an evidence-discipline gap, not a root-cause gap and not a schema gap — the behaviours QLoRA specialization was proposed to train — and it is not a routing gap (the failures are visible). It is a budget/latency gap: the cap set pre-registered for R6 stopped at the manifest ceiling (8192) and the contract forbids changing it after DEV.

**Does post-training remain justified?** Not for the strongest substrate on this evidence: Qwen3.8's behaviour when it completes is at the frontier's level on the classes it finishes, and its dominant failure mode is a budget the harness imposes. The live alternative the plan asks about is Nemotron: it retains a *systematic behaviour gap* (27 TEST silent failures — 16 evidence-only, 11 root cause) at one-fifth of Qwen3.8's latency, which is exactly the QLoRA target R5 proposed; it is deferred, not dismissed, because any Nemotron-centred system needs either training or a silent-failure detector R5 showed does not generalise, whereas budget calibration is cheaper, deterministic, and settles first whether the strongest local needs any training at all. Specialization of the cheap tier (Qwen3.5-9B / Bonsai evidence citation) is lower on all-pass than Qwen3.8 (not dominated: Bonsai has 14 TEST-only and Qwen3.5-9B 10 DEV-only passes vs Qwen3.8, and Qwen3.5-9B has the best local root-cause rate, 35/48) and the contract's gates rejected or floor-qualified them. **Favoured substrate if specialization is ever revisited:** Qwen3.8-27B Q3_K_M (`qwen3.8-27b-q3_k_m@7f3b845b5638` on `llama.cpp-upstream-9b05354@0e90f9139596`).

**Recommended next milestone (exactly one, not started):** **R7 — reasoning-budget calibration for the frozen Qwen3.8-27B execution system**: a pre-registered TRAIN-only study of caps beyond 8192 (e.g. 12288 at `-c 16384`, or a larger context) and, if the model's chat template supports it, a declared thinking-budget toggle — selected on TRAIN, frozen, then one DEV and one TEST under the same Suite v3, provenance registry and state machine, with the R4 replay and the latency/VRAM cost reported alongside. Its question is the one R6 leaves open: how much of the 49 % truncation converts to passes (16/47 were cut inside the answer and need only a few hundred tokens; the rest were still reasoning), at what wall time (truncated cases already take 275–280 s), and whether the UD-Q3_K_XL quant (39 % faster, frozen out by the quality-first rule at 8192) becomes the better operating point when the budget is no longer the binding constraint. Feasibility constraint to pre-register: at `-c 16384` Qwen3.8 already sits at 15.1–15.2 GiB of the 16.3 GiB card with q8_0 KV, and 3.6k prompt + a 12 288 cap (15.9k) barely fits — a larger context may not. Not QLoRA on Qwen3.8 on this evidence; not a router; Nemotron specialization is the fallback if R7 fails to convert the truncations.
...
| 7 | every new trajectory carries artifact + runtime/config identity | ✔ 516/516 components; 348/516 also the execution-system digest (pre-freeze pilot rows predate it) |
...
| 21 | answers whether Bonsai is a useful efficiency point | ✔ § 5.3, § 6 (qualifies on DEV; 0.67× memory on TEST; silent burden second only to Qwen3-8B on TEST) |
