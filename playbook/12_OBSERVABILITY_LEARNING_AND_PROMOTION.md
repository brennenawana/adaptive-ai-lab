...
| Dual clocks | Realtime *and* monotonic stamp on every span, plus periodic realtime-vs-monotonic offset samples | Virtualization/power-management clock skew is invisible until cross-checked (02, [SCENARIO: SCENARIO-12]) |
...
- [SCENARIO-05](examples/SCENARIO-05_hardware-purchase-discipline.md) — preserved per-invocation telemetry (finish reasons, session/host identity, dual clocks) is what let a later performance autopsy (06) reconstruct the timeline and run the counterfactual that refuted an intuitive hardware purchase; none of that reconstruction would have been possible without §5.1's floor already landed at the time the run happened.
- [SCENARIO-12](examples/SCENARIO-12_restart-instability-paired-controls.md) — dual-clock stamps surfaced a systematic clock-skew artifact that had been silently biasing every published latency; the fix followed directly from having captured both clocks, not from suspecting the skew in advance.
