...
The append-only record of every [look](../GLOSSARY.md#look) at a held-out
evaluation split — this playbook's operational form of the
[look ledger](../GLOSSARY.md#look-ledger). "TEST" in the file name names the
generic role (held-out evaluation data), not a specific split: an entry here can
be a look at the qualify split or the confirm split. An
[experiment contract](../GLOSSARY.md#experiment-contract) plans its look-ledger
entry at freeze (its §6 and Freeze Checklist); this document is where that plan
is actually kept, across every experiment that shares the split.
...
- **MUST** use from the first look at any held-out split — qualify or confirm —
  ...
  already made blind.
- **MUST** log every look, not only the ones that "mattered" — an unlogged look
  is unlogged exposure, and the [refresh trigger](#4-refresh-trigger) in §4 can
  only fire on what is recorded.
- **SHOULD** keep one ledger per project, not one per experiment — the resource
  being tracked is the split, which multiple experiment contracts share.
- **Do not** log iterate-split screening, racing, or successive-halving rungs
  here — that is [screening, not inference](../GLOSSARY.md#screening-vs-inference);
  it consumes iterate-split budget tracked in the experiment contract and never
  spends a held-out look.
...
*Fixed rule, not a per-project field — restated here so the ledger is
self-contained.*

A **look** is one arm's outcomes on a held-out split, read for a decision or
report:

- Reading *K* arms in the same sitting for one decision is *K* looks — one row
  per arm, because each arm's outcomes are separately produced and separately
  spendable.
- An **offline replay** (re-scoring or re-analyzing already-stored outputs)
  counts as a new look if it is a new read for a new decision or report. An
  analysis computed *within* an already-counted look — a second summary
  statistic pulled from the same stored outputs for the same decision — does
  not recount.
- A look is spent at its first executed item, not its last (see
  [spend semantics](../GLOSSARY.md#spend-semantics)) — the row is logged when
  the read starts, not after the result is known. There is no
  peek-and-decide-not-to-log.
- Rows are append-only. A look logged in error gets a correcting row, never an
  edit or deletion of the original.
...
*`#` is a permanent, append-only sequence — never renumbered or reused. `Items`
is the count of held-out items actually read, not the split's full size. A
`replay` row's `Notes` must name what changed (grader patch, re-analysis) and
confirm it serves the same decision as its parent look — otherwise it is a new
look, not a replay.*

### 3. Standing Consequences

*What is already spent, and what was decided from it — so a later reader does
not accidentally re-decide from an already-spent look.*
...
completes.` Choose `N`/`X` against the split size and expected number of
comparisons, and record the rationale here, not just the number.
...
[cluster](../GLOSSARY.md#clustering-unit), prefer adding new strata over
replicating items inside existing ones for the refresh: new strata buy
independent information toward [effective N](../GLOSSARY.md#effective-n);
replicating inside an already-represented cluster buys very little, because the
cluster, not the item, is the unit of information [EXT-STATS-001].
