...
A chapter tells you to do something and cites a vendor: `[FOLLOW: SRC-ID]` or
`[ADAPT: SRC-ID]`. Before you follow it, you want two things the vendor's own page
will not tell you — what its method quietly assumes, and whether the page is still
describing the product you are about to use. That is what this file is for.

There is an entry here for every source classified `FOLLOW` or `ADAPT` in
[sources.yaml](sources.yaml). `REFERENCE`- and `CASE`-classified sources live in
[sources.yaml](sources.yaml) and the examples library instead, and are skipped here
— except where a `REFERENCE`-tier source carries a trap sharp enough to be worth
warning about, and those are collected in
[§ Notable REFERENCE-tier traps](#notable-reference-tier-traps). Every
`DEPRECATED`-classified source appears in the standing
[§ Deprecation watchlist](#deprecation-watchlist) whether or not it is
trap-worthy, because the whole point of that section is that nobody should have to
rediscover a dead end twice.

**How to read an entry.** Each one has four fields, in the order you need them:

- **Solves** — what the source actually gives you, stated narrowly.
- **Does not solve** — the gap you must fill yourself, and where this playbook
  fills it.
- **Validate before relying on it** — the check that answers *does this fit my
  project?* This is the field people skip and the field that costs them. A recipe
  validated on someone else's task shape, model pairing, or data volume is a
  hypothesis about yours.
- **Version/pin & traps** — the staleness or mis-citation problem a reader of the
  raw document would not catch on their own: a doc tree that moves, a repository
  that was archived, a hosted product with a shutdown date, a figure that belongs
  to one benchmark and travels badly.

**How to read a verdict**, per [GLOSSARY: vendor verdicts](../GLOSSARY.md#vendor-verdicts):
**FOLLOW** = mature, apply the vendor's method as documented. **ADAPT** = mechanics
sound, decision layer missing — this file states exactly what to substitute. Both
carry an as-of date. Nothing below is evergreen; volatile and pre-alpha sources are
flagged explicitly.

**Where things are:** [evaluation, statistics and
judges](#evaluation-methodology-statistics--judges) · [experiment design, stopping
and measurement validity](#experiment-design-stopping--measurement-validity) ·
[fine-tuning, quantization and data](#fine-tuning-quantization--data) ·
[routing and cascades](#routing--cascades) · [inference performance and serving
engines](#inference-performance--serving-engines) · [deployment and
operations](#deployment--operations) · [agentic decision
framing](#agentic-decision-framing) · [legal and licensing](#legal--licensing) ·
[governance and process rubric](#governance--process-rubric) ·
[REFERENCE-tier traps](#notable-reference-tier-traps) · [deprecation
watchlist](#deprecation-watchlist) · [verification
...
- **Does not solve:** it does not choose your thresholds — tiers and drop limits are user-supplied. It requires **identical prompt templates** across compared runs, so it cannot evaluate a prompt change. And it assumes independent items, with no clustering/ICC correction — pair it with [EXT-STATS-001] whenever outcomes cluster.
- **Validate before relying on it:** confirm the doc-tree path resolves to current content at read time. Then confirm your own suite's clustering structure before trusting the reported MDE as-is.
...
- **Validate before relying on it:** confirm your output format matches the reference code's expectations. If you are adapting it to a different harness, re-derive the formulas from the paper rather than porting the code blind.
...
- **Does not solve:** it supplies derivations, not a ready-made tool. You implement against your own suite.
...
- **Does not solve:** it does not prescribe a versioning mechanism. Chapter 03's suite-release discipline is the playbook's answer to the finding.
- **Validate before relying on it:** n/a for citation; cite it as a finding about the process.
...
- **Does not solve:** a general contamination-detection method. The paper's own approach is to build a genuinely fresh comparison benchmark — heavy work, not a lightweight check you can run on demand.
- **Validate before relying on it:** n/a for citation. If you intend to replicate it, budget for actually building a fresh comparison set.
...
- **Does not solve:** it says nothing about judge quality on open-ended or preference tasks. That is a different domain — see the REFERENCE-tier MT-Bench and κ-deflation studies below.
...
- **Does not solve:** the OpenAI-specific tooling framing stops applying once the hosted product sunsets — see the trap below.
...
- **Solves:** a documented, numeric example of the payoff of human eval-set curation (1,699 candidate tasks reviewed down to a 500-task verified subset; corrected solve rate moved from 16% to 33.2% once decontaminated). Cite it as a worked instance of "curation moves the measured number," not as a live capability claim.
...
- **Does not solve:** it does not resolve the volume-vs-curation tension against EXT-EVAL-005's curation evidence. Chapter 03 keeps both positions visible with a decision rule — grading-signal quality × item cost × stakes tier — rather than picking a silent winner.
...
- **Does not solve:** it does not supply a stratification or ground-truth-design method past the taxonomy step.
- **Validate before relying on it:** n/a for citation.
...
- **Solves:** the convention — a unique marker plus a do-not-train sentence — for excluding published eval content from cooperative scrapers and training corpora. Direct input to a [canary string](../GLOSSARY.md#canary-string) policy.
- **Does not solve:** it does not stop non-cooperative scrapers. This is an honor-system convention, not an enforcement mechanism.
- **Validate before relying on it:** confirm which crawlers and aggregators actually honor the convention before treating it as a leakage control rather than a leakage detector.
...
- **Solves:** it models the practice of publishing a complete evaluation recipe (full configuration) alongside results, framed as "methodological consistency with clear provenance," not bit-wise reproducibility.
- **Does not solve:** **it does NOT document a pin-containers/sampling-params/judges discipline, and does NOT document a smoke-test discipline.** Two claims once attributed to this source were checked live and are absent under any wording variant tried. Do not cite this source for either.
...
- **Validate before relying on it:** if you are adopting alpha-spending directly, re-derive the spending function from a primary clinical-trials-statistics source. Do not import a specific numeric convention from this citation.
...
- **Does not solve:** it does not guarantee class balance. This playbook's addition is that rungs **must be class-balanced** (Bernstein racing applied at the class level), which the base literature does not require by default.
- **Validate before relying on it:** if you implement a rung schedule, verify your own implementation enforces stratum balance at every rung. A naive port of standard ASHA/Hyperband code will not do this automatically.
...
- **Does not solve:** it supplies no suite or task set of its own. You compute pass^k on your own declared subset.
...
- **Does not solve:** it does not bound *your* system's reproducibility. That requires your own restart/concurrency/cross-host probes (ch. 02), not an assumption that the paper's findings transfer numerically to your stack.
- **Validate before relying on it:** run your own reproducibility probes before scoping a comparability claim. Do not assume batch-invariant mode is enabled unless you have verified the flag or config yourself.
...
- **Does not solve:** it does not cover the conditions where fine-tuning wins — format, behavior, skill internalization, latency and cost at volume. Those are argued separately in chapter 09, not from this source.
- **Validate before relying on it:** check your task shape first. If your task is not factual/citation-shaped, this trio's conclusion may not transfer.
...
- **Solves:** it reconciles the LoRA-vs-full-FT quality contradiction by regime — all-layer LoRA approaches full-FT quality at post-training scale, at roughly two-thirds the FLOPs.
- **Does not solve:** it does not resolve the question in every regime. The earlier paper's single-model-scope caveat is the boundary condition; read both papers' scope statements before generalizing past post-training-scale, all-layer LoRA.
...
- **Does not solve:** it does not supply your regression suite. Pair it with chapter 03's [regression suite vs capability suite](../GLOSSARY.md#regression-suite-vs-capability-suite) distinction.
...
- **Does not solve:** sample-size / minimum-n guidance — an open field gap, stated as such in ch. 09. It also does not validate against your task's eval suite: these are generic starting points, not calibrated defaults.
- **Validate before relying on it:** treat rank and alpha as a [`[PARAMETER]`](../GLOSSARY.md#a-to-h-classification) to sweep on your iterate split, not a fixed value. Confirm the VRAM floors against your actual framework.
- **Version/pin & traps:** VRAM floor table matched EXT-FT-003 exactly at last check (9B: 6.5GB QLoRA / 24GB LoRA; 27B: 22GB / 64GB; 70B: 41GB / 164GB, stated as absolute minimums). Multi-GPU support status changed between checks — now "works, better version coming," previously "coming soon" only — so re-verify before depending on it. **Unsloth publishes no dates anywhere on its docs site**, which means you re-check the content itself each pass, not a timestamp.
...
- **Does not solve:** style-alignment-scale sizing, or narrow-behavioral-fix minimums (see the LIMA REFERENCE-tier trap note below). That remains an open field gap.
...
- **Does not solve:** generalization to non-tool-calling tasks. There is also no variance or confidence statement around the 93% figure.
- **Validate before relying on it:** treat the 93% as this tutorial's own result on its own task, not a transferable guarantee. Measure on your own tool-calling eval before citing an expected accuracy.
...
- **Does not solve:** **early-stopping numeric defaults (val_loss monitor, patience 10, min-delta 0.001) are NOT documented Customizer guidance.** They are code-level defaults in the open-source NeMo training framework, and whether the hosted Customizer microservice actually inherits them is unconfirmed.
- **Validate before relying on it:** **do not cite this source for the early-stopping numbers as published vendor doctrine.** If you adopt them, cite the framework source directly and treat "Customizer inherits these defaults" as an assumption to verify against your own run logs.
- **Version/pin & traps:** Kubernetes-footprint microservice — weigh the operational cost of the deployment target independently of the methodology question. This early-stopping misattribution is the single most important correction surfaced by live re-verification in this source set.
...
- **Solves:** a concrete, demonstrated quantization-acceptance bar — over 99% median accuracy recovery targeted for PTQ-only shipping; 95–99% is a *deliberate* PTQ design target (not a fallback) when a quantization-aware-distillation recovery stage is planned; plus a full worked example (96.33% → 99.72% median recovery, 10 of 11 benchmarks improved).
- **Does not solve:** it is demonstrated practice on the vendor's own model and benchmark suite, not published doctrine applicable by rule to arbitrary tasks. Re-derive the acceptance bar against your own task evals.
...
- **Version/pin & traps:** the page was 3 days old at verification — published, then checked days later — so re-verify before long-term citation. This is demonstrated-not-doctrine, per its `case-study` evidence-strength label.
...
- **Does not solve:** **it provides no evaluation procedure and no threshold.** The doc's own accuracy guidance is "we recommend running evaluations to verify... acceptable performance," with no dataset, metric, or acceptance bar attached.
...
- **Version/pin & traps:** **format is Blackwell-only** — an artifact produced with this recipe does not run on non-Blackwell hardware, so quantize for the deployment target, not the dev box (ch. 02). File last touched roughly one month before this check — re-verify before relying on stated timings.
...
- **Does not solve:** it is **pre-alpha, "Not for production use" per the repository's own warning**. It ships two-tier routing only, so three-tier routing must be assembled by chaining routes or writing a custom policy selector. An announced trained "prefill router" is not shipped — zero matching paths on the main branch at last check.
- **Validate before relying on it:** treat it as reference semantics for your own routing-policy vocabulary, not a production dependency, until the pre-alpha status changes.
- **Version/pin & traps:** a recent release was a substantial redesign (native server rewrite) that removed an earlier CLI/TUI/config-bundle surface — **if you are adapting example commands from an older reference, verify against the current release, not an older one.** Two-tier field names vary by route type, so do not assume one universal field-name pair; check the specific route type's schema.
...
- **Solves:** metric definitions (TTFT, ITL, system-vs-per-user TPS), ISL/OSL sweep design by use case, concurrency-over-request-rate sweeps, and operating-point selection on the latency-throughput curve. The most mature vendor methodology in this corpus, which is what warrants FOLLOW-by-the-book status.
- **Does not solve:** quality and capability evaluation — that is a separate instrument (ch. 03). It also does not isolate the single-user/interactive (concurrency=1) regime; the guide's sweep design does not cover it on its own. See [operating point](../GLOSSARY.md#operating-point).
- **Validate before relying on it:** confirm which doc version the guide resolves to at read time, because it rolls forward. Then re-run the guide's warmup/concurrency procedure against your own endpoint before trusting absolute numbers.
...
- **Does not solve:** it does not define acceptance thresholds. Pair it with your own operating-point selection (ch. 06).
...
- **Does not solve:** the source paper's headline figures were not independently re-verified in this pass, because the source PDF blocked automated fetch. Treat those specific numbers as unconfirmed; the concept is sound regardless.
...
- **Version/pin & traps:** no single tool release to pin — this is a methodology and a concept, not a versioned artifact.
...
- **Does not solve:** k6 itself is an HTTP load tool, not an LLM-aware benchmarking client. Pair it with AIPerf or NIM-guide metric definitions for token-level metrics.
...
- **Solves:** the de-facto standard open serving engine for the throughput regime. A deterministic batch-invariant mode exists, but it is opt-in.
...
- **Validate before relying on it:** measure your own reproducibility boundary — restart, concurrency and cross-host probes — rather than assuming determinism from engine choice alone.
...
- **Does not solve:** it does not supply a staged-gate procedure. Pair it with chapter 04's [screening vs inference](../GLOSSARY.md#screening-vs-inference) discipline to turn the table into a decision rule.
...
- **Solves:** it states plainly that quantization-aware distillation is the vendor's recommended accuracy-recovery strategy after quantization, and it is the only page in its repository that discusses the narrowest-precision format in this context.
...
- **Solves:** a documented production-observability floor: a Prometheus-compatible
...
- **Validate before relying on it:** confirm your serving stack actually passes
  engine-native metrics through rather than reinterpreting them, and adopt ONE
  metric vocabulary across all tiers.
- **Version/pin & traps:** the page updates frequently — it was last updated the day
  before verification — so re-check endpoint and env-var names against your
  deployed version.
...
- **Does not solve:** it does not address AI-specific deltas — nondeterministic outputs, verifier-in-the-loop quality metrics, provider-side model updates mid-canary. Those are this playbook's own additions (ch. 10, labeled `inference`).
...
- **Does not solve:** it supplies no machinery to act on the ordering. No stopping rules, no MDE, no gate mechanics; those are this playbook's own contribution.
...
**Standing rule for this entire group:** re-verify against the live terms **at
training time**, per project, never against a cached note. Provider terms are
amended independent of this playbook's release cycle, and the checks below show
they move on genuinely different clocks.

### EXT-LEGAL-001 — Anthropic Commercial Terms §D.4 + Usage Policy
*FOLLOW · verified 2026-08-21 · ch. 09, 13*

- **Solves:** it states plainly that the Commercial Terms (§D.4) and the separately-versioned Usage Policy both prohibit using model input/output to train competing models without authorization — the load-bearing legality check before any own-trace-harvesting plan touches this provider's outputs.
- **Does not solve:** it does not cover any other provider. Check each provider in your loop independently.
...
- **Solves:** the consumer Terms of Use prohibit using output to develop competing models; the Business Terms carry the same restriction with narrow Permitted Exceptions — non-distributed classifier/embedding models, and fine-tuning the provider's own offered services.
...
- **Validate before relying on it:** confirm which specific document governs your account type — consumer terms or Business Terms — and re-verify at training time.
...
- **Solves:** it states the same category of restriction — output may not be used to develop competing models — for the Gemini API surface.
...
- **Solves:** the 28-test rubric this playbook adopts as its periodic methodology self-audit, with the score computed as the **minimum** across the four category sections — not a sum or average — so a system must clear every category to raise its score.
- **Does not solve:** it does not define AI-specific failure classes beyond the two monitors this playbook borrows directly: training/serving skew, and holding out annotated production examples for regression.
...
- **Does not solve:** it does not specify what your gates should check. That is the reachability/gold-answer/determinism probe design in chapter 03.
...
- **Does not solve:** it does not supply a fine-tuning-forgetting regression suite. That is a different application of the same statistical pattern.
...
- **Solves:** a trajectory interchange format spanning multiple agent tools from one vendor's ecosystem. Adopt it if trajectories need to move between tools rather than living only in your own event log.
- **Does not solve:** it does not define what fields belong in a trajectory record for experimentation purposes. Chapter 12's minimum [trajectory record](../GLOSSARY.md#trajectory-record) is this playbook's own answer; the format is a wire format, not a field-completeness guarantee.
...
`REFERENCE`-classified sources are consult-don't-depend by definition and are not
given full entries above. The ones below carry traps sharp enough that a reader
relying on the chapter that cites them should know about the gap before they hit it.
...
- **Experiment-tracking dashboards lack tamper evidence.** Mainstream experiment-lineage tooling (metrics dashboards, artifact versioning) provides lineage-by-reference, but none of it is tamper-evident — adopt as a mirror or dashboard only, never as the [record of record](../GLOSSARY.md#record-of-record) (ch. 13).
...
- **Deterministic-inference mode has a real throughput cost.** At least one serving engine's deterministic-inference mode carries a documented 25–45% throughput slowdown, and is recommended by its own maintainers for debugging and reproducibility rather than steady-state serving — budget the cost before enabling it broadly.
...
Standing record of every source classified `DEPRECATED` in
[sources.yaml](sources.yaml). The moment to read it is before you cite an older
draft, follow a cached tutorial, or copy a command out of a search result that
might predate a supersession. Every one of these was found the hard way once; the
table exists so it is not found the hard way twice.

Each entry names **who is still exposed** — the situation in which the dead artifact
can still reach you — because most of these do not announce themselves. A withdrawn
repository still resolves; an old blog post still ranks; a deprecation notice can
sit on a branch nobody reads.

**A data-flywheel reference-loop blueprint** — withdrawn Apr 2026. *Replacement:*
none named; the loop design lives on conceptually in a newer integrated platform's
evaluate/tune pillars.
> **Still exposed if** you followed a link to it from another vendor doc or an older
> blog post — it is still cited and linked from both. Note what the artifact
> actually says: "reference only," not "the design is wrong." The
> log→curate→tune→judge→human-gate pattern remains worth copying even though the
> code is withdrawn.

**An LLM-routing blueprint** — notice added 2026-07-08. *Replacement:* a newer
routing library from the same vendor (see NV-SWITCHYARD-001, above).
> **Still exposed if** you are reading the `main` branch, which carries no notice at
> all — and `main` is the branch the notice's own comparison table treats as
> production. The deprecation notice lives only on the repository's `experimental`
> branch README, which happens to be that repository's *default* branch, so a casual
> visitor does see it.

**A GenAI-benchmarking client tool** — deprecation confirmed live 2026-08-21; no
specific date on the notice itself. *Replacement:* AIPerf (see NV-AIPERF-001, above).
> **Still exposed if** you are copying commands from the older four-part
> benchmarking blog series, which still shows the deprecated tool's commands with no
> banner. Check the current tool's migration guide before running anything from
> those posts.

**An RTX fine-tune-to-deploy toolkit** — archived 2025-11-21, final commit
2025-11-24. *Replacement:* none named.
> **Still exposed if** you are looking for a consumer-GPU fine-tune→deploy workflow
> from this vendor. This was the only official one, and it died with no successor
> documented anywhere in the corpus reviewed — plan for the gap rather than hunting
> for a replacement that does not exist.

**An earlier post-training alignment library** — deprecated and archived 2025-05-15.
*Replacement:* a newer post-training library from the same vendor.
> **Still exposed if** you are following any pre-2025 alignment tutorial, since those
> reference the older library and are superseded along with it. Verify any older
> alignment doc against the current library's API before following it.

**A TensorRT-LLM serving backend for a managed inference microservice** — dropped
from a production release branch, released May 2026. *Replacement:* a vLLM-based
backend for the same microservice.
> **Still exposed if** you are working from a device-specific playbook corpus, which
> still ships a variant using the deprecated backend alongside current playbooks.
> Its presence there does not mean the backend is current for the general product.

**An older model-optimization toolkit name and docs domain** — rebrand announced
2025-12-08, effective at the next minor version. *Replacement:* the renamed
toolkit's current docs domain.
> **Still exposed if** you updated one link type and not the other. The old
> documentation subsite hard-404s, while the old **repository** slug transparently
> redirects via host-level rename handling — the two behave differently, which is
> exactly what makes it easy to miss.

**An older evaluation-SDK documentation tree** (adapters/interceptors/parameters
pages) — superseded by the current SDK's docs; restructure confirmed live
2026-08-21. *Replacement:* the current SDK's docs, served from a non-versioned
"nightly" path.
> **Still exposed if** you are citing a "latest" path and assuming it is pinned. It
> 308-redirects to the "nightly" path, and **there is no separately pinned "latest"
> doc tree at all**; a specific-version static path 404s. Cite the "nightly" path
> explicitly with an as-of date, never "latest" as if it were stable.

**An older post-training-quantization example script** — superseded at a specific
toolkit minor version. *Replacement:* a newer example script in the same repository.
> **Still exposed if** you are following an old tutorial link, which lands on a bare
> 404 with no pointer to the replacement. The old script's directory 404s, and the
> supersession is inferred from that 404 plus the current directory listing —
> **no in-repo prose migration note exists**.

**A branded "cloud" product name as a directly-rentable destination** —
repositioned 2026. *Replacement:* separately-branded rentable marketplace products
from the same vendor.
> **Still exposed if** you are relying on an older portability promise — "move
> seamlessly to the cloud" — which now names a destination that no longer presents
> as a rentable product at all. The original brand is now vendor-internal, and the
> actual rentable destinations have no documented local on-ramp (see the
> REFERENCE-tier trap note above).

---

## Verification discipline

- Every verdict, date, and quoted phrase in this file was re-verified live against
  the primary source during this authoring pass — as of 2026-08-21 unless a section
  states an individual date otherwise. Re-verify volatile and snapshot-tier sources
  again at the start of the *next* authoring pass before reusing a verdict. A vendor
  verdict recorded once and never rechecked is itself an unpriced escape hatch (see
  ch. 00 §9, "unpriced escape hatches").
- **Snapshot-tier sources cite an as-of date at the point of use, always** — most
  importantly the hardware/cloud price snapshot chapter 11's worked examples draw
  on. Prices verified live during a hardware supply crunch carry a shelf life
  measured in weeks, not months. A price without a printed as-of date next to it
  should be treated as untrustworthy regardless of source.
- Prefer a direct fetch of the primary source over any automated summary of it. This
  pass caught two summarization failures on load-bearing sources: a paper's authors
  and publication venue misattributed by a first-pass automated summary of a PDF,
  and a code-repository release timeline misread by two years by the same kind of
  summary. Both were caught only by re-reading the primary document directly or
  cross-checking with an independent tool. Treat any single-pass automated summary
  of a primary source as a hypothesis to verify, not a citation to ship.
- A source's `classification` field in [sources.yaml](sources.yaml) is the record of
  record. This file's job is to explain *why* that classification holds and what a
  reader must still supply on top of it. If the two ever disagree, sources.yaml wins
  and this file has a bug — flag it rather than trusting whichever you read first.
