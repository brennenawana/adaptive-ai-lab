...
"Copperfield Labs," an internal ML-platform team, runs a nightly regression
evaluation for a document-classification pipeline plus occasional multi-day
fine-tuning sprints. Rented cloud GPU spend has started to feel steady
enough that someone keeps proposing a workstation-class GPU purchase.
...
The rent-vs-buy archetype from chapter 11's decision tree: no privacy
mandate forces local ownership, demand is real but not yet proven sustained,
and the decision is a reversible capex question rather than a safety or
compliance one. That places it at **Tier 2** — enough rigor to require the
demand ledger and a pre-committed trigger, not enough to require anything
from chapter 13's Tier-3 set.

## The decisive moves

1. **The demand ledger opened before any purchase conversation.** Every
   GPU-hour — rented and any incidental local use — was logged monthly from
   the start, including honest NOT-RUN rows for jobs that were planned but
   blocked, so a busy week during a crunch could not later be misread as
   sustained demand ([demand ledger](../GLOSSARY.md#demand-ledger), chapter
   11).
2. **The purchase trigger was written down before anyone wanted the
   hardware.** Pre-committed rule: the
   ...
   break-even threshold H* — not from any single month, however busy.
3. **H* was computed with the standard formula, symbols and units stated,
   before looking at a single month of ledger data.**
...
4. **[STOP CONDITION]** Fired mid-project, and held. After one crunch month
   (below), a team member proposed buying immediately. This is the global
   tripwire on hardware-purchase decisions made without the demand ledger's
   own trigger ([00 §7](../00_PRINCIPLES_AND_SCOPE.md)). The proposal was
   held pending the quarter-end ledger review rather than argued on the spot.
5. **The ledger, not the anecdote, made the call.** At quarter-end the
   rolling 3-month average sat well under H*; the trigger did not fire; the
   team correctly did not buy. Renting continues, and the same pre-committed
   trigger stands for reassessment next quarter — it was never loosened
   after failing to fire once.
6. **The benchmark-first rule was recorded for whenever the trigger does
   ...
   quoted — hardware and cloud prices in a tight supply market are floors
   with a shelf life of weeks, not a fixed number [EXT-HW-001].

## What was skipped and why

- **Parallel-node / critical-path analysis (11).** Current jobs are
  single-GPU-bound; the marginal-node-value question (does a second GPU
  actually shorten the critical path, or just add idle capacity) does not
  arise yet and is deferred until a job is demonstrably parallelizable.
- **Capacity-vs-bandwidth hardware-class comparison.** Nothing here is a
  bandwidth-bound serving workload; a single mid-tier high-VRAM GPU would
  cover both the eval and fine-tuning use cases, so the serving-class
  hardware comparison was not engaged.
- **The local-only-mandate branch of the decision tree.** No regulated data
  is in scope, so the privacy-forced-ownership path never applies; rented
...
GPU-hours, $62; Month 4, 16 GPU-hours, $52 — including one honestly logged
NOT-RUN row for an 8-hour job blocked two days by a data-pipeline defect.
Weekly-average utilization: roughly 3.2, 14.1, 4.4, and 3.7 hours/week. Only
Month 2 exceeded the 10.1 hr/week H* threshold, and not for three
consecutive months — the pre-committed trigger never fired. Total rented
spend across the period: $358, against a $4,000 purchase that would have sat
mostly idle. The team kept renting, kept the ledger running, and left the
trigger exactly as pre-committed for the next quarterly review. The purchase
correctly does not happen yet.
